...
카카오채널 포스트 목록은 스크롤 직후 게시물이 즉시 생성되지 않을 수 있다. **빠르게 연속 스크롤하면 다음 게시물이 로드되기 전에 DOM을 검사하여 `상품 없음`, `게시물 없음` 또는 `목록 끝`으로 잘못 판단할 수 있다.**

다음 규칙을 반드시 지킨다.

1. 한 번에 페이지 끝까지 급하게 스크롤하지 않는다. 화면 높이의 약 70~90%씩 작은 단계로 내린다.
2. 한 번 스크롤한 뒤 새 게시물 로딩 판정이 끝나기 전에는 다음 스크롤을 실행하지 않는다.
3. 각 스크롤 뒤 아래 신호 중 하나를 확인할 때까지 기다린다.
   - 새로운 `post_id` 발견
   - 게시물 카드 개수 증가
   - 로딩 표시가 나타났다가 사라짐
   - 관련 네트워크 요청 완료 후 DOM이 안정됨
4. 위 신호를 즉시 확인할 수 없어도 최소 1~2초의 완충 시간을 둔 뒤 DOM을 다시 검사한다.
5. 신규 게시물이 한 번 나오지 않았다는 이유만으로 목록 끝이라고 판단하지 않는다. 같은 위치에서 최대 3회까지 `대기 → DOM 재검사 → 소폭 추가 스크롤`을 수행한다.
6. 최초 접속 시 게시물 카드가 0개이면 빈 목록으로 확정하지 않는다. 최대 10~15초 동안 단계적으로 기다리며 카드 컨테이너와 링크를 다시 확인한다.
7. `상품 없음`, `게시물 없음`, `목록 끝` 판정은 로딩 대기와 제한 재시도를 모두 수행한 뒤에만 내린다.
8. 고정된 짧은 시간 대기만 믿지 말고 신규 ID, 카드 수, 로딩 표시, 네트워크 완료를 함께 사용한다.

금지 사항:

- 대기 없이 `scroll → scroll → scroll → DOM 검사`를 연속 실행
- 스크롤 직후 카드가 0개라는 이유로 빈 목록 처리
- 단 한 번 신규 ID가 없다는 이유로 수집 종료
- 아직 로딩 중인데 `end_post_not_found` 또는 목록 끝으로 확정
...
- 위 로딩 대기 규칙과 제한 재시도를 모두 수행했는데도 신규 `post_id`가 추가되지 않음
